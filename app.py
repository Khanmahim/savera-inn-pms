        ...
        if not filtered:
            ...
            for b in reversed(filtered):
                ...
                with st.expander("📋 More details"):
                    ...
                    if b.get('meal_price', 0):
                        d1.write(f"🍽️ **Meal Rate:** {fmt(b.get('meal_price', 0))}/person/day")
                    d2.write(f"🛏️ **Extra Bed:** {'Yes' if b.get('extra_bed') else 'No'}")
                    d2.write(f"🔥 **Bonfire:** {'Yes' if b.get('bonfire') else 'No'}")
                    if b.get('advance_paid', 0):
                        d2.write(f"💳 **Advance Paid:** {fmt(b.get('advance_paid', 0))} ({b.get('advance_method', 'Cash')})")
                    else:
                        d2.write("💳 **Advance Paid:** None")
    ...
    with tab2:
        ...
        if not available_rooms:
            ...
            with st.form("add_booking_form"):
                ...
                c7, c8 = st.columns(2)
                extra_bed = c7.checkbox("🛏️ Extra Bed?")
                ...
                if submitted:
                    if not guest:
                        ...
                        persist()
                        msg = f"✅ Booking confirmed for **{guest}** — Room {rnum} | {checkin} → {checkout}"
                        if extra_bed:         msg += " | 🛏️ Extra Bed"
